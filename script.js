...
for (i = 0; i < answerSelected.length; i++) {
    answerSelected[i].addEventListener('click', (event) => {

        const randomQuestionId = event.target.id;
        checkAnswers(randomQuestionId);
    });
...
function checkAnswers(randomQuestionId) {

    // add to score and alert if answer is correct
    if (randomQuestionId === questionArray[questionNumber].correctAnswer) {
        correctCount++;
        alert("Correct!");
